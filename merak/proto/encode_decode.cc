...
namespace merak {
    ...
    std::pair<std::string,bool> change_camel_style(std::string_view field) {
        bool changed = false;
        std::string result;
        auto it = field.begin();
        auto end = field.end();

        auto constexpr as_it_is = [](char c, std::string_view::const_iterator it,  std::string_view::const_iterator end) {
            return it == end ? true :
                   c != '_' ? true :
                   !turbo::ascii_isalpha(*it) ? true :
                   turbo::ascii_isupper(*it) ? true: false;
        };
        while (it != end) {
            auto c = *it++;
            /// put as it is
            if(as_it_is(c, it, end)) {
                result.push_back(c);
                continue;
            }
            result.push_back(turbo::ascii_toupper(*it++));
            changed = true;
        }
