...
{\
    Document d1, d2;\
    d1.parse(json1);\
    ASSERT_FALSE(d1.has_parse_error());\
    d2.parse(json2);\
    ASSERT_FALSE(d2.has_parse_error());\
...
{\
    EXPECT_TRUE(expected2 == schema.GetError().object_empty());\
...
{\
    EXPECT_TRUE(schema.GetError().object_empty());\
    EXPECT_TRUE(schema.IsSupportedSpecification());\
...
TEST(SchemaValidator, ValidateMetaSchema) {
    ...
    Document d;
    d.parse(json);
    ASSERT_FALSE(d.has_parse_error());
    SchemaDocument sd(d);
        ...
        if (!json) {
            ...
            if (d.has_parse_error()) {
                ...
                for (Value::ConstValueIterator schemaItr = d.begin(); schemaItr != d.end(); ++schemaItr) {
                    {
                        const char* description1 = (*schemaItr)["description"].get_string();
...
TEST(SchemaValidatingReader, Simple) {
    ...
    EXPECT_TRUE(d.is_string());
    EXPECT_STREQ("red", d.get_string());
}
...
TEST(SchemaValidatingWriter, Simple) {
    ...
    EXPECT_STREQ("\"red\"", sb.get_string());
...
TEST(Schema, Issue848) {
    ...
    merak::json::SchemaDocument s(d);
    merak::json::GenericSchemaValidator<merak::json::SchemaDocument, merak::json::Document> v(s);
}
...
// Fail to resolve $ref in allOf causes crash in SchemaValidator::start_object()
...
TEST(SchemaValidator, Issue1017_allOfHandler) {
    ...
    Writer<StringBuffer> writer(sb);
    GenericSchemaValidator<SchemaDocument, Writer<StringBuffer> > validator(s, writer);
    EXPECT_TRUE(validator.start_object());
    EXPECT_TRUE(validator.emplace_key("cyanArray2", 10, false));
    EXPECT_TRUE(validator.start_array());
    EXPECT_TRUE(validator.end_array(0));
    EXPECT_TRUE(validator.emplace_key("blackArray", 10, false));
    EXPECT_TRUE(validator.start_array());
    EXPECT_TRUE(validator.end_array(0));
    EXPECT_TRUE(validator.end_object(0));
    EXPECT_TRUE(validator.IsValid());
    EXPECT_STREQ("{\"cyanArray2\":[],\"blackArray\":[]}", sb.get_string());
}
...
TEST(SchemaValidator, Ref_internal_multiple_ids) {
    ...
    Document sd;
    sd.parse(schema);
    ASSERT_FALSE(sd.has_parse_error());
...
TEST(SchemaValidator, UnknownValidationError) {
    ASSERT_TRUE(SchemaValidator::SchemaType::GetValidateErrorKeyword(kValidateErrors).get_string() == std::string("null"));
}
...
TEST(SchemaValidator, DuplicateKeyword) {
    ...
    EXPECT_FALSE(sd.has_parse_error());
    SchemaDocument s(sd);
    VALIDATE(s, "42", true);
    INVALIDATE(s, "\"Life, the universe, and everything\"", "", "type", "",
        "{ \"type\": {"
...
TEST(SchemaValidator, Schema_SupportedNotObject) {
    ...
    EXPECT_TRUE(s.GetError().object_empty());
}
...
TEST(SchemaValidator, Schema_ReadOnlyAndWriteOnly) {
    ...
    EXPECT_TRUE(s1.GetError().object_empty());
