...
TEST(Allocator, MemoryPoolAllocator) {
    const size_t capacity = MERAK_JSON_ALLOCATOR_DEFAULT_CHUNK_CAPACITY;
...

    {
        ...
        const size_t bufSize = 1024;
        char *buffer = static_cast<char *>(a.Malloc(bufSize));
        MERAK_JSON_ASSERT(bufSize % sizeof(void*) == 0);
...
TEST(Allocator, Alignment) {
    if (sizeof(size_t) >= 8) {
        EXPECT_EQ(MERAK_JSON_UINT64_C2(0x00000000, 0x00000000), MERAK_JSON_ALIGN(0));
        for (uint64_t i = 1; i < 8; i++) {
            EXPECT_EQ(MERAK_JSON_UINT64_C2(0x00000000, 0x00000008), MERAK_JSON_ALIGN(i));
            EXPECT_EQ(MERAK_JSON_UINT64_C2(0x00000000, 0x00000010), MERAK_JSON_ALIGN(MERAK_JSON_UINT64_C2(0x00000000, 0x00000008) + i));
            EXPECT_EQ(MERAK_JSON_UINT64_C2(0x00000001, 0x00000000), MERAK_JSON_ALIGN(MERAK_JSON_UINT64_C2(0x00000000, 0xFFFFFFF8) + i));
            EXPECT_EQ(MERAK_JSON_UINT64_C2(0xFFFFFFFF, 0xFFFFFFF8), MERAK_JSON_ALIGN(MERAK_JSON_UINT64_C2(0xFFFFFFFF, 0xFFFFFFF0) + i));
        }
    }

    EXPECT_EQ(0u, MERAK_JSON_ALIGN(0u));
    for (uint32_t i = 1; i < 8; i++) {
        EXPECT_EQ(8u, MERAK_JSON_ALIGN(i));
        EXPECT_EQ(0xFFFFFFF8u, MERAK_JSON_ALIGN(0xFFFFFFF0u + i));
    }
}
